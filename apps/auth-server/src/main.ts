...
// Create token for new user
  ...
  let authenticatedUser = getAuthenticatedUser({email, password});
  res.status(200).json({authenticatedUser,access_token})
})
...
server.post('/auth/login', (req, res) => {
  ...
  console.log(req.body);
  const {email, password} = req.body;
  let authenticatedUser = isAuthenticated({email, password});
